...
function run (args) {
    ...
    try {
        // Pass notification signals through to the child, but let's not kill ourselves
        [ "SIGHUP", "SIGUSR1", "SIGUSR2" ].forEach( function(signal) {
            process.on(signal, function () {
                var child = exports.child;
                if (child) {
                    log("Received "+signal+", notifying child process...");
                    child.kill(signal);
                }
            });
        });
